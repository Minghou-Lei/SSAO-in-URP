    ...
    half smoothness, half occlusion, half3 emission, half alpha)
{
    ...
    float2 screenPos = ComputeScreenPos(TransformWorldToHClip(inputData.positionWS));
    float2 uv = screenPos / _ScreenParams.xy;

    
    float indirectAmbientOcclusion = SAMPLE_TEXTURE2D_X(_ScreenSpaceOcclusionTexture, sampler_ScreenSpaceOcclusionTexture, uv).x;
    float directAmbientOcclusion = lerp(1.0, indirectAmbientOcclusion, 0.2);

    mainLight.color *= directAmbientOcclusion;
